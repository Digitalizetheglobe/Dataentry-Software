...
const Deposit = () => {
  ...
  const handleDelete = async () => {
    try {
      ...
      await Promise.all(
        selectedEntries.map((id) => axios.delete(`http://localhost:8000/api/deposit-withdraw/delete-entry/${id}`))
      );
    ...
    <>
      ...
      <div className="max-w-5xl mr-1 mx-auto justify-between items-center mb-8 ml-10 rounded ">
        ...
        <div className="flex items-center mb-6 ml-10 mt-10">
          ...
          <h2 className="text-lg font-semibold text-gray-700 ml-2">Deposit Entries</h2>
          <div className="flex items-center space-x-4">
          </div>
        </div>
